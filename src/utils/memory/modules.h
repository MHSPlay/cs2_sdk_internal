...
namespace PE {
    ...
    public:
        __forceinline c_module( ) :
            m_ldr_entry{ nullptr }, m_dos{ nullptr }, m_nt{ nullptr } {}

        __forceinline c_module( LDR_DATA_TABLE_ENTRY *ldr_entry, IMAGE_DOS_HEADER *dos, IMAGE_NT_HEADERS *nt ) :
    ...
    using modules_t = std::vector< c_module >;
    ...
    static __declspec( noinline ) c_module GetModule( std::string name ) {
        modules_t modules;
    ...
    static __declspec( noinline ) c_module GetModule( const hash32_t name ) {
        modules_t modules;
    ...
    static __declspec( noinline ) Address GetExport( const c_module &module, const std::string &name );

    // gets the address of an exported function in a module by hash.
    static __declspec( noinline ) Address GetExport( const c_module &module, const hash32_t name ) {
	...
	static __declspec( noinline ) exports_t GetAllExports( const c_module &module ) {
		Address     export_ptr, fwd_export_ptr;
		size_t      delim;
		std::string export_name, fwd_str, fwd_module, fwd_export;
		exports_t   exports;
